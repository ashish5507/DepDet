...
│         EEG BRANCH              │    │        SPEECH BRANCH             │

│                                 │    │                                  │

│  Raw EEG (128ch x 500 samples)  │    │   Raw audio (.wav per subject)   │

│              ↓                  │    │              ↓                   │

│  Preprocessing (MNE-Python)     │    │  Resample to 16kHz               │

│  - Bandpass filter 1-40Hz       │    │  Trim silence (VAD)              │

│  - ICA artifact removal         │    │              ↓                   │

│  - Epoch: -200ms to +800ms      │    │  librosa feature extraction      │

│    around dot onset             │    │  - 13 MFCCs (mean + std)         │

│              ↓                  │    │  - Delta MFCCs (mean + std)      │

│  Explicit ERP features          │    │  - Delta-delta MFCCs             │

│  - P300 amplitude (sad, fear,   │    │  - Pitch mean + std              │

│    happy conditions)            │    │  - Energy (RMS) mean + std       │

│  - P300 latency per condition   │    │  - Speaking rate (ZCR)           │

│  - P200 amplitude               │    │              ↓                   │

│  - Frontal alpha asymmetry      │    │  StandardScaler normalization    │

│  - Frontal theta power          │    │              ↓                   │

│              ↓                  │    │  Speech feature vector           │

│  EEGNet-8,2 encoder             │    │  (~83 numbers per subject)       │

│  - Conv2D (1x125) F1=8          │    └──────────────┬───────────────────┘

│  - DepthwiseConv (128x1) D=2    │                   │

│  - SeparableConv + AvgPool      │                   │
...
└──────────────┬──────────────────┘                   │
               ...
               └──────────────┬───────────────────────┘
                   ...
                   (~108 numbers per subject)

                              ↓

                   MLP Fusion Classifier

                   Dense(64) + BatchNorm + ELU + Dropout(0.5)

                   Dense(32) + BatchNorm + ELU + Dropout(0.5)
